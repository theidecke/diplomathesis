	...
	\subsection{Elimination der Neumann--Reihe}
	Unsere Darstellung der L"osung des Strahlungstransportproblems (\ref{eq:STGpathsolution}) besteht bisher aus einer unendlichen Summe von Integralen, was eine direkte Folge der Aufspaltung des L"osungsoperators aus (\ref{eq:op.STGinvert}) in seine Neumann--Reihe (\ref{eq:op.STGneumann}) ist. In dieser Form sind die Pfade in Klassen gleicher L"ange (d.h. Anzahl an Streupunkten) eingeteilt, was die mathematische Behandlung verinfacht, da die F"alle verschiedener L"ange getrennt voneinander behandelt werden k"onnen. Gleichzeitig verleitet diese Darstellung jedoch dazu zu glauben, es w"are sinnvoll oder l"age in der Natur der Sache, die Pfade unterschiedlicher L"ange getrennt voneinander zu behandeln. Tats"achlich gibt es aber keinen physikalischen Grund daf"ur, vielmehr w"are es auch mathematisch hilfreich die L"osung als ein einziges Integral "uber den Raum aller Pfade darzustellen, da wir es dann formal mit nur noch einem Integrationsproblem zu tun h"atten und nicht mit unendlich vielen.
	...
	als {\em Messbeitragsfunktion}, dann k"onnen wir nun die polychromatische Messung (\ref{eq:messgleichung_frommonos}) einfach als
	\begin{equation}
		M=\int_\Omega f({\overline x})d\mu({\overline x})
		\label{eq:unified_measurementintegral}
	\end{equation}
	schreiben. Es ist in dieser Form leichter einzusehen, da"s wir die Pfade nicht nach ihrer L"ange zusammengefasst behandeln m"ussen, um anschliessend die (unendlich vielen) Ergebnisse aufzusummieren, sondern stattdessen Pfade in beliebiger Weise zur Integration zusammenfassen d"urfen. Dies ist insbesondere f"ur die Monte--Carlo--Integration wichtig, da unsere ganze Theorie sich dort auf die L"osung eines Einzelintegrals beschr"anken wird und wir dort dann nahtlos mit der L"osung von Messintegralen der Form (\ref{eq:unified_measurementintegral}) ankn"upfen k"onnen.
